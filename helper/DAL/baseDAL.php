...
class baseDAL
{
    ...
    function get_data_list($Table, $Field = "*", $Where = 1, $Order = NULL)
    {
        $SQL = new sqlclass;
        $BDTable = strtolower($Table);
        $DBField = ucfirst($BDTable);
        ...
        if ($Field <> "*") {
            #Just make sure that the ID shows up somewhere...
            if (!strpos($Field, $DBField)) {
                $Field = "ID" . $DBField . "," . $Field;
            }
        }
        if (is_null($Order)) {
            #Default ordering by ID DESC
            $Order = "ID" . $DBField . " DESC";
        }

        $Req = "SELECT " . $Field . " FROM " . $BDTable . " WHERE " . $Where . " ORDER BY " . $Order;
        $SQL->Select($Req);

        $Ret = array();
        while ($Rep = $SQL->FetchAssoc()) {
            $Ret[$Rep["ID" . $DBField]] = $Rep;
        }
        return $Ret;
    }
