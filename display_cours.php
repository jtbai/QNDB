...
foreach($Piscine as $v){
	$MainOutput->OpenRow();
		...
		$now = time();
		$Req = "SELECT DISTINCT Niveau, Jour, Start, round((End-Start)/60) as Duree, min(cours.IDCours) as IDCours , min(ressource.IDEmploye) as Full FROM piscine JOIN niveau JOIN cours JOIN periode JOIN ressource ON cours.IDCours = ressource.IDCours AND periode.IDPiscine= piscine.IDPiscine AND cours.IDPeriode=periode.IDPeriode AND cours.IDNiveau = niveau.IDNiveau  WHERE piscine.IDPiscine=".$v['IDPiscine']." AND periode.IDSession = ".$_ACTIVE['Session']." AND (Semaine+Jour*(3600*24)+Start-".$now.")>0 GROUP BY periode.IDPiscine, Jour, Start, cours.IDNiveau ORDER BY Jour ASC, Start ASC, Rank ASC";
			$SQL->SELECT($Req);
		$OldStart = 0;
		$OldJour = -1;
