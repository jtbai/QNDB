...
if($_POST['FORMArgent']<>""){
	...
	$NB = $SQL->NumRow();
	if($NB==0){
		$Req = "INSERT INTO timesheet(`IDEmploye`,`Salaire`,`Heures`,`IDPaye`,`Ajustement`,`Role`) VALUES('".$_POST['FORMIDEmploye']."','".$_POST['FORMArgent']."','0','".$IDPaye."',1,'$')";
	}else{
		$Rep = $SQL->FetchArray();
	...
	if($_POST['FORMTaux']=='Autre' || $_POST['FORMTX']<>""){
		...
		$NB = $SQL->NumRow();
	if($NB==0){
		$Req = "INSERT INTO timesheet(`IDEmploye`,`Salaire`,`Ajustement`,`Heures`,`IDPaye`,`Role`) VALUES('".$_POST['FORMIDEmploye']."','".$Salaire."','".$_POST['FORMNBH']."','1','".$IDPaye."','".$Role."')";
	}else{
		$Rep = $SQL->FetchArray();
